...
SERVICE_ACCOUNT_EMAIL="deploy-bot@attendancerecords.iam.gserviceaccount.com"
...
# 1. Autenticación en Google Cloud con el email de la cuenta de servicio
log "Autenticando con la cuenta de servicio $SERVICE_ACCOUNT_EMAIL..."
gcloud auth login $SERVICE_ACCOUNT_EMAIL --brief || { log "Error en la autenticación"; exit 1; }
...
# 7. Despliegue en Cloud Run con conexión a Cloud SQL
